...
const PublicProfile = ({ match }) => {
  ...
  const { loading, error, eraseError, profile, fetchPublicProfile } =
    publicContext;
    ...
    <>
      ...
      {profile && (
        <Container className="bg-light pt-2 pb-3 border col-sm-12 col-lg-6">
          {profile.isPublic ? (
            <Badge variant="success" className="me-3 mt-2">
              public profile
            </Badge>
          ) : (
            <Badge variant="danger" className="me-3 mt-2">
              non-public profile
            </Badge>
          )}
          {profile.publicNotesExist && (
            <Link to={`/user/${profile.profileName}/notes`}>See Notes</Link>
          )}
          {profile.isPublic && (
            <Row>
