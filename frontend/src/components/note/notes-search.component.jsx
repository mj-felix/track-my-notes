...
const SearchNotes = ({ history, location }) => {
    ...
    const handleClick = (tagId, eraseKeyword) => {
        ...
        history.push(`/notes?tags=${searchCriteria.tags.join()}&search=${eraseKeyword ? '' : keyword}`);
    };
        ...
        <Card>
            ...
            <Card.Body>
                ...
                {tags.map(tag => {
                    ...
                    const isTagSelected = searchCriteria.tags ? searchCriteria.tags.includes(tag._id) : false;
                    return (
                        <Button
                            key={tag._id}
