...
// user1.fileKey0, .fileKey1, .url0 - to be used for file's upload and deletion
...
describe("User registration", () => {
  ...
  it("fails when no profile name, email and password provided", async () => {
    ...
    expect(response.body.errors[0].reason).to.eql(
      errors.user.PROFILE_NAME_REQUIRED
    );
    expect(response.body.errors[1].reason).to.eql(errors.user.EMAIL_REQUIRED);
    expect(response.body.errors[2].reason).to.eql(
      errors.user.PASSWORD_REQUIRED
    );
  });
  ...
  it("fails when invalid profile name, email or password provided", async () => {
    ...
    expect(response.body.errors[2].field).to.eql("password");
    expect(response.body.errors[0].reason).to.eql(
      errors.user.INVALID_PROFILE_NAME
    );
  ...
  it("succeeds when valid profile name, email or password provided", async () => {
    ...
    expect(response.body.user).to.not.include.all.keys(
      "password",
      "refreshToken"
    );
...
describe("User profile update", () => {
  ...
  it("succeeds when correct access token provided", async () => {
    ...
    expect(response.body.linkedIn).to.eql(
      "https://www.linkedin.com/in/mjfelix/"
    );
...
describe("Notes", () => {
  ...
  it("multiple notes' retrieval successful (inc. order, search and pagination)", async () => {
    ...
    const payload3 = {
      ...
      tags: [user1.tag1, user1.tag2, user1.tag3],
    };
    const payload4 = {
      title: "Note 4",
      link: "Note 3.pl",
    ...
    expect(noteIds).to.have.ordered.members([
      user1.note4,
      user1.note1,
      user1.note3,
      user1.note2,
    ...
    expect(response.body.pages).to.eql(1);
    expect(response.body.notes.map((note) => note._id)).to.have.ordered.members(
      [user1.note4, user1.note1, user1.note3]
    );
  });
});
...
describe("Files", function () {
  ...
  it("upload successful", async () => {
    ...
    expect(response.body.files[0].originalFileName).to.eql(
      "Thank_You_Kitty.png"
    );
    expect(response.body.files[0].bucket).to.eql("track-my-notes-dev");
    expect(response.body.files[0].url).includes(
      "https://track-my-notes-dev.s3"
    );

    // data setup
    user1.fileKey0 = response.body.files[0].storedFileName;
    user1.url0 = response.body.files[0].url;

    // WHEN:
    response = await require("supertest")(user1.url0).get("");
  ...
  it("deletion successful", async () => {
    ...
    expect(response.body.files[0].originalFileName).to.not.eql(
      "Thank_You_Kitty.png"
    );
...
describe("Public content", () => {
  it("successful public profile retrieval", async () => {
    ...
    // THEN:
    expect(response.status).to.eql(200);
    expect(response.body).to.not.include.any.keys(
      "password",
      "refreshToken",
      "isAdmin"
    );
  ...
  it("successful public notes retrieval (inc. order, search and pagination)", async () => {
    ...
    response = await request.get(
      `${endpoint}?pageSize=2&page=2&tags=${user1.tag1}`
    );
    ...
    // WHEN:
    response = await request.get(
      `${endpoint}?pageSize=5&page=1&search=note%203&tags=${user1.tag2}`
    );
